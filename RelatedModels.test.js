...
const models = new RelatedModels(modelDefs);

let product1, product2, product3;
beforeAll(async () => {
  product1 = models.create('product', {
    name: 'Burger',
    price: 10,
  });
  product2 = models.create('product', {
    name: 'Water',
    price: 2.5,
  });
  product3 = models.create('product', {
    name: 'Ice Cream',
    price: 3,
  });
});
...
describe('one2many/many2one', () => {
  describe('create', () => {
    it('automatically adds orderline to the order', () => {
      const order1 = models.create('order', {});
      const orderline1 = models.create('orderline', {
        order_id: order1.id,
  ...
  describe('update', () => {
    it('adds to one2many field', () => {
      ...
      const order1 = models.create('order', {});
      models.update('order', order1.id, {
        orderline_ids: [{ type: 'add', items: [orderline1.id] }],
      });
      expect(order1.orderline_ids.includes(orderline1)).toBe(true);
      expect(orderline1.order_id).toBe(order1);
    });
    it('replaces one2many field', () => {
      ...
      const order1 = models.create('order', { orderline_ids: [orderline1.id] });
      const orderline2 = models.create('orderline', {
        product_id: product2.id,
        quantity: 4,
      });
      const orderline3 = models.create('orderline', {
        product_id: product1.id,
        quantity: 1,
      });
      models.update('order', order1.id, {
        orderline_ids: [
          ...
          {
            type: 'add',
            items: [orderline2.id, orderline3.id],
          },
        ],
      });
    ...
    it('updates properly after series of updates', () => {
      const ol1 = models.create('orderline', {
        product_id: product1.id,
        quantity: 1,
      });
      const order1 = models.create('order', {
        orderline_ids: [ol1.id],
      });
      ...
      expect(orderline1.order_id).toBe(order1);
      const ol = models.create('orderline', {
        product_id: product3.id,
        quantity: 3,
      });
      models.update('order', order1.id, {
        orderline_ids: [{ type: 'add', items: [ol.id] }],
      });
      expect(order1.orderline_ids.length).toBe(3);
      models.delete('orderline', [orderline1.id]);
      expect(order1.orderline_ids.length).toBe(2);
      models.update('order', order1.id, {
        orderline_ids: [
          {
            type: 'remove',
            items: order1.orderline_ids.map((line) => line.id),
          },
        ],
      });
    ...
    it('updates the related model', () => {
      ...
      const product1 = models.create('product', {
        name: 'product1',
        price: 10,
      });
      const product2 = models.create('product', { name: 'product2', price: 5 });
      const orderlines = models.createMany('orderline', [
        { product_id: product1.id, quantity: 3 },
        { product_id: product2.id, quantity: 2 },
      ]);
      const order1 = models.create('order', {
        orderline_ids: orderlines.map((line) => line.id),
      });
      ...
      models.update('product', product1.id, { price: 100 });
  ...
  describe('delete', () => {
    ...
    beforeEach(() => {
      order1 = models.create('order', {});
      orderline1 = models.create('orderline', {
        order_id: order1.id,
        product_id: product1.id,
        quantity: 1,
      });
...
describe('many2one without inverse to the related type', () => {
  it('removes the association to productA when productA is deleted', () => {
    const productA = models.create('product', { name: 'productA', price: 100 });
    const orderline1 = models.create('orderline', {
      product_id: productA.id,
      quantity: 10,
    });
    expect(orderline1.product_id).toBe(productA);
    models.delete('product', [productA.id]);
...
describe('many2many', () => {
  describe('create', () => {
    it('creates w/ ids given to the many2many field', () => {
      const productA1 = models.create('product', {
        name: 'Product A1',
        price: 10,
      });
      const productA2 = models.create('product', {
        name: 'Product A2',
        price: 100,
      });
      const tagA = models.create('tag', {
        name: 'Tag A',
        product_ids: [productA1.id, productA2.id].sort(),
      });
    ...
    //   const productA = models.create('product', {
    //     name: 'Product A',
    //     price: 10,
    //     tag_ids: [{ name: 'Tag 1' }, { name: 'Tag 2' }],
    //   });
    //   const [tag1, tag2] = models.read(
    //     'tag',
  ...
  describe('update', () => {
    it('updates a record using "replace" mode', () => {
      ...
      const productB = models.create('product', {
        name: 'productB',
        price: 100,
      });
      const tag1 = models.create('tag', {
        name: 'Tag 1',
        product_ids: [productA.id, productB.id].sort(),
      });
      ...
      const productC = models.create('product', {
        name: 'productB',
        price: Infinity,
      });
      models.update('tag', tag1.id, {
        product_ids: [
          { type: 'clear' },
          {
            type: 'add',
    ...
    it('updates a record using "add" mode', () => {
      const tag1 = models.create('tag', { name: 'tag1' });
      const productA = models.create('product', {
        name: 'productA',
        price: 5,
        tag_ids: [tag1.id],
      ...
      expect(productA.tag_ids).toEqual([tag1]);
      const tag2 = models.create('tag', { name: 'tag2' });
      models.update('product', productA.id, {
        tag_ids: [
          {
            type: 'add',
            items: [tag2.id],
    ...
    it('updates a record using "remove" mode', () => {
      ...
      expect(productC.tag_ids).toEqual([tag1]);
      models.update('tag', tag1.id, {
        product_ids: [{ type: 'remove', items: [productB.id] }],
      });
      expect(tag1.product_ids.map((product) => product.id).sort()).toEqual(
  ...
  describe('delete', () => {
    it('updates the related records after deletion 1', () => {
      ...
      models.deleteMany('product', [productA.id, productC.id]);
    ...
    it('updates the related records after deletion 2', () => {
      ...
      const tag2 = models.create('tag', {
        name: 'tag2',
        product_ids: [productB.id, productC.id],
      });
      const tag3 = models.create('tag', {
        name: 'tag3',
        product_ids: [productA.id, productC.id],
      });
      ...
      models.deleteMany('tag', [tag1.id, tag2.id]);
      expect(productA.tag_ids).toEqual([tag3]);
      expect(productB.tag_ids).toEqual([]);
      expect(productC.tag_ids).toEqual([tag3]);
      models.deleteMany('product', [productA.id, productB.id]);
...
describe('many2many without corresponding relation_ref', () => {
  ...
  it('works when updating using add mode', () => {
    ...
    expect(orderline1.tax_ids).toEqual([tax1]);
    const tax2 = models.create('tax', {
      name: 'tax2',
      percentage: 50,
    });
    models.update('orderline', orderline1.id, {
      tax_ids: [{ type: 'add', items: [tax2.id] }],
    });
  ...
  it('works when updating using delete mode', () => {
    ...
    models.update('orderline', orderline1.id, {
      tax_ids: [{ type: 'remove', items: [tax2.id] }],
    });
  ...
  it('works when updating using replace mode', () => {
    ...
    models.update('orderline', orderline1.id, {
      tax_ids: [{ type: 'clear' }, { type: 'add', items: [tax1.id, tax3.id] }],
    });
    expect(orderline1.tax_ids.map((tax) => tax.id).sort()).toEqual(
  ...
  it('works when deleting', () => {
    ...
    const orderline1 = models.create('orderline', {
      ...
      product_id: product1.id,
      quantity: 1,
      tax_ids: [tax1.id, tax2.id, tax3.id],
    });
    expect(orderline1.tax_ids.map((tax) => tax.id).sort()).toEqual(
      [tax3.id, tax1.id, tax2.id].sort()
    );
    models.deleteMany('tax', [tax2.id, tax3.id]);
    expect(orderline1.tax_ids.map((tax) => tax.id)).toEqual([tax1.id]);
  });
});
